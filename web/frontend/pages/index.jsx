...
import { Button, Card, Heading, Image, Layout, Link, Page, Stack, TextContainer, TextField } from '@shopify/polaris';
...
export default function HomePage() {
  const htmlInject = '<div>Header title from API</div>';
  const fetch = useAuthenticatedFetch();

  const handleInjectHTML = async () => {
  ...
  return (
    <Page narrowWidth>
      ...
      <Layout>
        ...
        <Layout.Section>
          <ProductsCard />
          <div>Add header title to shopify theme</div>
          <TextField label='Header title' value={htmlInject} />
          <Button onClick={handleInjectHTML}>Click to add</Button>
        </Layout.Section>
      </Layout>
    </Page>
  );
